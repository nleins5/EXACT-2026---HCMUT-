...
├── fallbacks.py            # Giữ kết quả solver khi explanation model fail/thiếu budget
...
| `logic_solver` | restricted subprocess | Z3 code | True/False/Unknown |
| `logic_explanation` | Instruct | solver output | ExactResponse |
| `physics_rag` | Optional Embedding + Reranker | question | formulas + examples hoặc empty context |
| `physics_formalizer` | Coder | question, RAG context | SymPy code |
| `physics_solver` | restricted subprocess | SymPy code | numerical answer |
