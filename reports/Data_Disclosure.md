...
|---|---:|---|---|
| EXACT 2026 official release (May 15) | Type 1: 411 records / 808 questions; Type 2: 1,352 problems | Fine-tuning, validation, format definition, transparent exact-match retrieval for released examples | [ura.hcmut.edu.vn/exact](https://ura.hcmut.edu.vn/exact). Parsed locally; the updated release already removes invalid Type 2 QA rows. Ambiguous duplicate inputs are excluded from retrieval. |
...
- No external RAG corpus or persisted vector database is active. Runtime retrieval is limited to transparent exact matching over the disclosed released EXACT corpus; ambiguous duplicate and unseen inputs continue through the reasoning pipeline.
